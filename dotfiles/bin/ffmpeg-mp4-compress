...
# NOTE(toiletbril): Taken from some stackoverflow discussion I forgot, and
# edited by me.
...
if ! [ "$1" -a "$2" ]; then
    echo "USAGE: $0 <file> <desired size in MB>"
    exit 1
fi
...
echo "Minimal size is $T_MINSIZE MB"

echo "Calculating target bitrate..."
...
echo "Target bitrate is $T_VRATE KiB/s"
...
# Get rid of .mbtree and logs
if [ -s "$_PWD/ffmpeg2pass-0.log" -a -s "$_PWD/ffmpeg2pass-0.log.mbtree" ]; then
    rm "$_PWD/ffmpeg2pass-0.log" "$_PWD/ffmpeg2pass-0.log.mbtree"
fi
